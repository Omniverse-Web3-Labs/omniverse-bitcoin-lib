...
export function inscribe(data: string) {
    ...
    switch (netType) {
        case Network.Mainnet: {
            ret = execSync(`ord ${netParam} wallet inscribe --fee-rate 2 --file .inscription.json`);
            break;
        }
        case Network.Regtest: {
            ret = execSync(`ord ${netParam} --bitcoin-rpc-pass=b --bitcoin-rpc-user=a wallet inscribe --fee-rate 2 --file .inscription.json`);
            break;
        }
        case Network.Testnet: {
            ret = execSync(`ord ${netParam} --rpc-url ${bitcoin.getProvicer()} --bitcoin-rpc-pass=btc2023 --bitcoin-rpc-user=btc wallet inscribe --fee-rate 2 --file .inscription.json`);
            break;
        }
    }
    return ret;
 ...
 */
export function subscribe(p: InscriptionSubscribeParams, cb: (data: Array<any>[], blockHash: string, blockHeight: bigint) => void) {
    return ordinals.subscribe(p, (block) => {
        ...
        cb(rets, block.hash, block.height);
    });
}
